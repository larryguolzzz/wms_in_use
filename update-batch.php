...
//echo "文件存储在: " . "upload/" . $_SESSION['daifabatchname'].".csv". "<br>";
                ...
                if (@$content && @sizeof(@$content) == 28) {
                    try {
                        ...
                        $tosend_array = [];
                        while (@$content = fgetcsv($filepath)) {    //每次读取CSV里面的一行内容      
                            $sql = "UPDATE daifaorders SET carrier='DHL Express', service='DHL Express', tracking='" . $content[1] . "'WHERE (cmpid='" . $cmpid . "') AND orderid='" . $content[5] . "'";
                            $result = mysqli_query($conn, $sql);
                            $sql = "SELECT daifaorders.note from daifaorders WHERE daifaorders.orderid='" . $content[5] . "'";
                            $result = mysqli_query($conn, $sql);
                            $row = mysqli_fetch_array($result);

                            if ($_POST['checkbox'] != NULL) {
                                $pattern = '/(([a-zA-Z0-9]|\-|\.|\s|[()]|\/)+\*)/';  //匹配inote里的整数
                                preg_match_all($pattern, $row[0], $matchsku);
                                $pattern = '/(\*\d+(;|；))/';  //匹配inote里的整数
                                preg_match_all($pattern, $row[0], $match);
                                for ($ind = 0; $ind < count($matchsku[0]); $ind++) {
                                    $prex = str_replace("*", "", $matchsku[0][$ind]);
                                    $prey = str_replace("*", "", $match[0][$ind]);
                                    $prey = str_replace(";", "", $prey);
                                    $prey = str_replace("；", "", $prey);
                                    $tosend[$indexofcontent]['ordernumber'] = $content[5];
                                    $tosend[$indexofcontent]['marketplacesku'] = $prex;
                                    $tosend[$indexofcontent]['amount'] = $prey;
                                    $tosend[$indexofcontent]['market'] = $_POST['mkt'];
                                    $tosend[$indexofcontent]['tracking'] = $content[1];
                                    $tosend[$indexofcontent]['ship'] = 'DHL';
                                    if (array_key_exists($prex, $tosend_array)) {
                                        @$tosend_array[$prex] += $prey;
                                    } else {
                                        @$tosend_array[$prex] = $prey;
